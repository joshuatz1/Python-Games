# pacman
'''
working on adding the dots to eat
'''
import pygame as pg
...
font = pg.font.Font("assets/fonts/Neogen-Black.ttf", 54)

playerIMG = pg.image.load("assets/images/pacman7.jpg")
playerIMG = pg.transform.scale(playerIMG, (70, 40))
...
class masterClass():
    ...
    def __init__(self,x,y,color,speed):
        self.hitbox = pg.Rect(x,y,self.w,self.h)
        self.color = color
        self.speed = speed
        self.xspeed = 0
        self.yspeed = 0
...
#creating instace/object of the class masterClass
player = masterClass(50,50,"orange",10)

aliens = []
spacing = 85

#now a for loop to create as many aliens in this case 5, from 0-4
'''
for alien in range(0,12):
    print(alien)
    for alienCol in range(0,10):
        print(alienCol)
        anAlien = masterClass(60+alienCol*spacing, 60+alien*spacing+5,"yellow",7)
        aliens.append(anAlien)
...
for alien in range(0,12):
    ...
    for alienCol in range(0,10):
        ...
        if alien == 10  :
            anAlien = masterClass(alienCol*spacing+30, alien*spacing+100,"green",10)
        else:
            anAlien = masterClass(60+alienCol*spacing, 60+alien*spacing-15,"yellow",7)
        #if alienCol == 3  or alienCol == 4  or alienCol == 5 or alienCol == 6:
            #anAlien = masterClass(50+alienCol*spacing, 200+alien*spacing,"yellow",10)
            #continue
        aliens.append(anAlien)
    ...
    if Menu:
        ...
        for alien in aliens[:]:   # iterate over a copy
            ...
            for wall in allWalls:
                if alien.collision(wall):
                    print("collided")
                    aliens.remove(alien)
                    break

        for i in allWalls:
            pg.draw.rect(screen, "purple", i)
            if player.colliderect(i):
                #running = False
                pass
            '''if alien.colliderect(i):
                alien = False'''
